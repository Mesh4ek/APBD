...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps
            .Select(emp => new { emp.EName, Total = emp.Sal + (emp.Comm ?? 0) })
            .ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            ...
            .SelectMany(ed => grades
                .Where(grade => ed.emp.Sal >= grade.Losal && ed.emp.Sal <= grade.Hisal)
                .Select(grade => new { EName = ed.emp.EName, DName = ed.dept.DName, Grade = grade.Grade }))
            .ToList();
